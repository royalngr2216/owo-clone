...
class System(commands.Cog):
    ...
    async def leaderboard(self, ctx):
        async with ctx.typing():
            ...
            top_entries, top_ids = [], set()
            for index, user in enumerate(top_docs):
                try: user_id = int(user["user_id"])
                except (TypeError, ValueError, KeyError): continue
                top_ids.add(user_id); cash = user.get("cash", 0)
                try:
                    fetched_user = await self.bot.fetch_user(user_id); name = getattr(fetched_user, "display_name", fetched_user.name); avatar_url = str(fetched_user.display_avatar.url)
                except Exception:
                    name = f"User {user_id}"; avatar_url = "https://cdn.discordapp.com/embed/avatars/0.png"
                top_entries.append({"rank": index + 1, "name": name, "cash": cash, "user_id": user_id, "avatar_url": avatar_url, "title_key": get_equipped(user_id)})
            if not top_entries:
                await ctx.send(embed=discord.Embed(description="❌ No one has any cash yet.", color=0xED4245)); return
            requester_entry = None
            if ctx.author.id not in top_ids:
                my_doc = economy_collection.find_one({"user_id": str(ctx.author.id)}); my_cash = my_doc.get("cash", 0) if my_doc else 0
                if my_cash > 0:
            ...
            try:
                buf = await render_leaderboard(top_entries, requester_entry, format_cash); await ctx.send(file=discord.File(buf, filename="leaderboard.png"))
            except Exception:
                embed = discord.Embed(title="LEADERBOARD", color=0xF1C40F); embed.description = "\n\n".join(f"**#{entry['rank']} {entry['name']}**\n{format_cash(entry['cash'])}" for entry in top_entries); await ctx.send(embed=embed)
    ...
    async def stop(self, ctx):
        ...
        for games in (randoms_games, deathroll_games, crack_games):
            if ctx.channel.id in games: del games[ctx.channel.id]; stopped = True
        await ctx.send(embed=discord.Embed(description="🛑 Active game stopped." if stopped else "❌ No active game.", color=0xED4245))

...
class Blackjack(commands.Cog):
    ...
    async def blackjack(self, ctx, amount: str = None):
        if ctx.author.id in bj_games:
            await ctx.send(embed=discord.Embed(title="⚠️ Action Required", description="You already have a blackjack game in progress!\nPlease finish it using the buttons below.", color=COLOR_LOSS))
            return
        if amount is None:
            embed = discord.Embed(title="🃏 Premium Blackjack", description=("### The Rules\n> Beat the dealer to **21** without going over.\n> Dealer hits on **16 or below**, stands on **17+**.\n> Natural Blackjack pays **1.5×**.\n\n### Commands\n` .bj <amount> ` or ` .bj all `\n"), color=COLOR_PLAYING)
        ...
        if cash < bet:
            ...
            return
        # No maximum gambling/bet limit: players can wager any amount they can afford.
        ...
        bj_games[ctx.author.id] = {"player": player, "dealer": dealer, "deck": deck, "bet": bet, "done": False}
    ...
    def _build_embed_view(self, user, log_text):
        ...
        img = render_blackjack(player, dealer, ptotal, 0, True, bet, format_cash)
        file = discord.File(img, filename="blackjack.png")
        embed.set_image(url="attachment://blackjack.png")
        embed.set_footer(text=f"Hit, Stand, or Double • {BLACKJACK_TIMEOUT}s per turn")
    ...
    def _build_animation_embed(self, user, player, dealer, bet, log_text):
        ...
        embed.set_author(name=f"{user.display_name}'s Blackjack Game", icon_url=user.display_avatar.url)
        embed.description = f"> {log_text}"
        img = render_blackjack(player, dealer, ptotal, dtotal, False, bet, format_cash)
        file = discord.File(img, filename="blackjack.png")
        embed.set_image(url="attachment://blackjack.png")
    ...
    async def _finalize_ui(self, msg, user, player, dealer, bet, result, log_text, interaction=None):
        ...
        dtotal = hand_total(dealer)
        if result in ["win", "bj"]:
            color, icon = COLOR_WIN, "✅"
        elif result == "loss":
            color, icon = COLOR_LOSS, "❌"
        else:
            color, icon = COLOR_DEALER, "🤝"
        embed = discord.Embed(color=color)
        embed.set_author(name=f"{user.display_name}'s Blackjack Game", icon_url=user.display_avatar.url)
        embed.description = f"> {icon} **{log_text}**"
        img = render_blackjack(player, dealer, ptotal, dtotal, False, bet, format_cash, result=result)
        file = discord.File(img, filename="blackjack.png")
        embed.set_image(url="attachment://blackjack.png")
    ...
    async def _end_game(self, interaction, user_id, result_type):
        ...
        dtotal = hand_total(dealer)
        if ptotal > 21:
            outcome, final_log = "loss", f"You busted! Lost {format_cash(bet)}."
        elif dtotal > 21:
            outcome, final_log = "win", f"Dealer busted! You won {format_cash(bet)}."
        elif ptotal > dtotal:
            outcome, final_log = "win", f"You beat the dealer! Won {format_cash(bet)}."
        elif ptotal == dtotal:
            outcome, final_log = "push", f"It's a tie! Bet of {format_cash(bet)} returned."
        else:
            outcome, final_log = "loss", f"Dealer wins. Lost {format_cash(bet)}."
        if outcome == "win":
...
class BjView(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        try:
            msg = g.get("msg")
            if msg:
                embed = discord.Embed(title="⏳ Game Lost", description=f"You didn't make a move within **{BLACKJACK_TIMEOUT} seconds**.\n\n💸 Bet lost: **{format_cash(g['bet'])}**", color=COLOR_LOSS)

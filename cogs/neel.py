...
class Neel(commands.Cog):
    ...
    async def neel(self, ctx):
        ...
        embed.set_footer(
            text="Catches have a 10% chance of being stolen by Neel"
        )
    ...
    async def neel_sell(self, ctx, *, pokemon_name: str = None):
        ...
        if not pokemon_name:
            await ctx.send(embed=discord.Embed(
                description=(
                    "**Usage:** `.neel sell <Pokémon>`\n"
                    "**Example:** `.neel sell Rayquaza`"
                ),
                color=0xED4245,
            ))
            return
        ...
        embed = discord.Embed(
            title="💰 DEAL COMPLETE",
            color=RARITY_EMBED_COLORS.get(rarity, NEEL_COLOR),
        )

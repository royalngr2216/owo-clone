...
class Economy(commands.Cog):
    ...
    async def _transfer_cash(self, ctx, member, amount_text):
        ...
        if sender_balance < amount:
            await ctx.send(f"❌ You don't have enough NGR. You have **{format_cash(sender_balance)}**.")
            return
        if receiver_balance + amount > MAX_BALANCE:
            remaining = MAX_BALANCE - receiver_balance
            await ctx.send(f"❌ That would put {member.mention}'s balance over the **{format_cash(MAX_BALANCE)}** limit.\nMaximum you can give them right now: **{format_cash(max(0, remaining))}**.")
            return

        economy_collection = __import__("utils.economy", fromlist=["economy_collection"]).economy_collection
        result = economy_collection.update_one(
        ...
        receiver_result = economy_collection.update_one(
        ...
        embed = discord.Embed(
            title="💸 MONEY SENT",
            description=f"{ctx.author.mention} sent **{format_cash(amount)}** to {member.mention}.\n\n💵 Your balance: **{format_cash(sender_balance - amount)}**",
            color=0x57F287,
        )
        brand(embed)
        await ctx.send(embed=embed)
